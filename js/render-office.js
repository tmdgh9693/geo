...
// OFFICE RENDER
function drawOffice(){
 const sky=ctx.createLinearGradient(0,0,0,H);sky.addColorStop(0,'#244a66');sky.addColorStop(1,'#0b2234');ctx.fillStyle=sky;ctx.fillRect(0,0,W,H);
 // playful blocky building shell
 ctx.fillStyle='#243847';rr(40,104,1840,910,28);ctx.fill();ctx.fillStyle='#dbe5e9';rr(62,126,1796,866,24);ctx.fill();ctx.fillStyle='#b9c8cf';rr(62,126,1796,70,24);ctx.fill();
 // zones
 ctx.fillStyle='#c9d6dc';rr(88,220,560,420,22);ctx.fill();ctx.fillStyle='#c3d6d2';rr(705,220,395,710,22);ctx.fill();ctx.fillStyle='#cbd5da';rr(1145,220,690,710,22);ctx.fill();ctx.strokeStyle='rgba(31,76,128,.10)';ctx.lineWidth=4;ctx.stroke();
 text('업무구역 · OPERATIONS',106,205,20,'#203849','left',1000);text('출동 준비구역 · READY BAY',730,205,20,'#176356','left',1000);text('정비창고 · MAINTENANCE STORAGE',1170,205,20,'#203849','left',1000);
 // window sea
 ...
 // briefing area
 ctx.fillStyle='#f4fbff';rr(98,676,532,266,22);ctx.fill();ctx.fillStyle='#a0d8ff';rr(122,726,180,112,16);ctx.fill();ctx.fillStyle='#9ebeff';rr(332,726,220,112,16);ctx.fill();ctx.fillStyle='#ffe48c';rr(122,856,430,54,14);ctx.fill();text('브리핑 / 휴게 공간',116,706,15,'#203849','left',900);text('현재 신고: '+(currentMission?currentMission.title:'신고 대기'),132,891,13,'#4d5a00','left',900);
 // ready bay
 ...
 // warehouse blocks
 function rack(x,y,w,h,label,c1,c2){ctx.fillStyle=c1;rr(x,y,w,h,18);ctx.fill();ctx.fillStyle=c2;rr(x+12,y+44,w-24,h-56,14);ctx.fill();ctx.fillStyle='rgba(35,74,146,.12)';for(let yy=y+74;yy<y+h-36;yy+=96)ctx.fillRect(x+24,yy,w-48,14);text(label,x+18,y+28,15,'#203849','left',1000)}
 ...
 // blocky player
 {const wp=player.walkPhase||0,bob=player.moving?Math.sin(wp*2)*2:0,step=player.moving?Math.sin(wp)*8:0;ctx.save();ctx.translate(player.x,player.y+bob);ctx.scale(player.facing||1,1);ctx.fillStyle='rgba(7,17,25,.20)';ctx.beginPath();ctx.ellipse(0,28,23,8,0,0,TAU);ctx.fill();ctx.strokeStyle='#243342';ctx.lineWidth=8;ctx.lineCap='round';ctx.beginPath();ctx.moveTo(-8,23);ctx.lineTo(-10+step*.35,40);ctx.moveTo(8,23);ctx.lineTo(10-step*.35,40);ctx.stroke();ctx.fillStyle='#e8eef0';rr(-16,-6,32,31,7);ctx.fill();ctx.fillStyle='#f5b840';rr(-16,-9,32,8,4);ctx.fill();ctx.fillStyle='#263f50';rr(-13,4,26,11,3);ctx.fill();ctx.fillStyle='#d7a57b';ctx.beginPath();ctx.arc(0,-20,12,0,TAU);ctx.fill();ctx.fillStyle='#f2c94c';ctx.beginPath();ctx.arc(0,-24,14,Math.PI,TAU);ctx.fill();ctx.strokeStyle='#e8eef0';ctx.lineWidth=6;ctx.beginPath();ctx.moveTo(-14,2);ctx.lineTo(-24-step*.22,13);ctx.moveTo(14,2);ctx.lineTo(24+step*.22,13);ctx.stroke();ctx.restore();text('점검원',player.x,player.y-47+bob,11,'#163243','center',800)}
