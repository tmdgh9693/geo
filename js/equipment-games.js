...
let lastEquipmentMiniGameNames=[];
...
function pickEquipmentMiniGames(inventoryItems, count = 3) {
  ...
  let shuffled=shuffleCopy(pool);
  if(pool.length>count&&lastEquipmentMiniGameNames.length){
    const fresh=shuffled.filter(x=>!lastEquipmentMiniGameNames.includes(x.name));
    const repeated=shuffled.filter(x=>lastEquipmentMiniGameNames.includes(x.name));
    shuffled=[...fresh,...repeated];
  }
  const picked=shuffled.slice(0,Math.min(count,pool.length));
  lastEquipmentMiniGameNames=picked.map(x=>x.name);
  return picked;
}
...
function equipmentMiniGameBody(item) {
  ...
  if (cfg.type === 'cables') return `<div class="cableGame"><div class="cableSide">${['빨강','파랑','노랑'].map((c,i)=>`<button class="cableEnd c${i}" data-cable="${i}">${c} 케이블</button>`).join('')}</div><div class="cableArrow">→</div><div class="cableSide">${shuffleCopy([0,1,2]).map(n=>`<button class="cableSocket c${n}" data-socket="${n}">${['빨강','파랑','노랑'][n]} 단자</button>`).join('')}</div></div><div class="gameStatus" id="equipmentStatus">같은 색의 케이블과 단자를 연결하세요.</div>`;
  ...
  if (cfg.type === 'solar') return `<div class="solarBox"><div class="solarMeter"><div class="solarPointer" id="equipSolarPointer"></div></div><input class="solarRange" id="equipSolarRange" type="range" min="0" max="100" value="${8+Math.floor(Math.random()*85)}"><div class="solarReadout" id="equipSolarReadout">충전 상태 15%</div><button class="btn" id="equipSolarCheck" style="margin-top:10px">충전 상태 확인</button></div><div class="gameStatus" id="equipmentStatus">초록 범위에 맞춘 뒤 확인하세요.</div>`;
  ...
  if (cfg.type === 'lens') return `<div class="lensAlign"><div class="lensAxis"></div><div class="lensTargetRing"><div class="lensSweetSpot">OK</div></div><div class="lensDisc" id="lensDisc">🔆</div></div><div class="lensScale"><span>왼쪽</span><div class="lensScaleTrack"><div class="lensGoodZone"></div><div class="lensScaleNeedle" id="lensScaleNeedle"></div></div><span>오른쪽</span></div><input class="solarRange lensRange" id="lensRange" type="range" min="0" max="100" value="${Math.random()>.5?8+Math.floor(Math.random()*27):66+Math.floor(Math.random()*27)}"><div class="lensReadout" id="lensReadout">정렬 정확도 36%</div><button class="btn" id="lensCheck">이 위치로 고정</button><div class="gameStatus" id="equipmentStatus">렌즈가 초록색 중심 범위에 들어오면 고정하세요.</div>`;
  ...
  if (cfg.type === 'blueprint') {const opts=shuffleCopy([{t:'등명기 → 전원함 → 고정부',ok:false},{t:'전원함 → 등명기 → 고정부',ok:true},{t:'고정부 → 등명기 → 전원함',ok:false}]);return `<div class="blueprintGame"><div class="blueprintPaper"><div class="bpTarget">등명기</div><div class="bpTarget">전원함</div><div class="bpTarget">고정부</div></div><div class="bpChoices">${opts.map(o=>`<button data-blueprint data-correct="${o.ok?'1':'0'}">${o.t}</button>`).join('')}</div></div><div class="gameStatus" id="equipmentStatus">도면에서 연결 구조에 맞는 순서를 골라보세요.</div>`;}
  if (cfg.type === 'pattern') {const target=variant,hints=['길게 한 번 점등 후 쉬는 패턴','짧게 두 번 깜빡인 뒤 쉬는 패턴','짧게 세 번 빠르게 깜빡이는 패턴'];return `<div class="patternDemo"><div class="patternHint">예시: ${hints[target]}</div><div class="patternRow">${['A','B','C'].map((n,i)=>`<button class="patternChoice" data-equip-pattern="${i}" data-correct="${i===target?'1':'0'}"><div class="patternLamp ${i===0?'a':i===1?'b':'c'}"></div>${n} 패턴</button>`).join('')}</div></div><div class="gameStatus" id="equipmentStatus">예시와 같은 점멸을 찾아보세요.</div>`;}
  if (cfg.type === 'clean') return `<div class="cleanLens" id="equipCleanLens">${[0,1,2,3,4].map(()=>{const x=18+Math.floor(Math.random()*58),y=15+Math.floor(Math.random()*58),r=-30+Math.floor(Math.random()*60);return `<button class="dirtSpot" data-equip-dirt style="left:${x}%;top:${y}%;transform:rotate(${r}deg)"></button>`}).join('')}</div><div class="cleanCount" id="equipCleanCount">남은 오염 5개</div>`;
...
function bindEquipmentMiniGame(item, onSuccess) {
  ...
  if (cfg.type === 'battery') {
    ...
    let gauge=5+Math.random()*88,dir=Math.random()>.5?1:-1,targetMin=24+Math.random()*32,targetMax=targetMin+20;const good=document.querySelector('.gaugeGood');if(good){good.style.left=targetMin+'%';good.style.width=(targetMax-targetMin)+'%'} clearRepairTimer(); repairTimer=setInterval(()=>{if(gamePaused||gameOver)return;gauge+=dir*(2.2+Math.random()*.5);if(gauge>94){gauge=94;dir=-1}if(gauge<4){gauge=4;dir=1}const n=$('equipGaugeNeedle');if(n)n.style.left=gauge+'%'},35); $('equipMeasureBtn').onclick=()=>{if(gauge>=targetMin&&gauge<=targetMax){clearRepairTimer();done()}else if(status())status().textContent='조금만 다시! 바늘이 초록 범위일 때 눌러보세요.'};
    ...
    document.querySelectorAll('[data-blueprint]').forEach(b=>b.onclick=()=>{if(b.dataset.correct==='1'){b.classList.add('ok');done()}else if(status())status().textContent='도면의 전원함에서 등명기로 이어지는 구조를 다시 살펴보세요.'});
  } else if (cfg.type === 'pattern') {
    document.querySelectorAll('[data-equip-pattern]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-equip-pattern]').forEach(x=>x.classList.remove('selected'));b.classList.add('selected');if(b.dataset.correct==='1')done();else if(status())status().textContent='예시 점멸과 같은 리듬을 다시 찾아보세요.'});
    ...
    let x=8+Math.floor(Math.random()*34),y=58+Math.floor(Math.random()*32);const marker=$('equipMapMarker');
